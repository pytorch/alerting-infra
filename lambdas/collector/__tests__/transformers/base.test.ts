...
describe("BaseTransformer", () => {
  ...
  describe("extractPriority", () => {
    ...
    it("should throw error for empty priority", () => {
      expect(() => transformer["extractPriority"]("")).toThrow(
        "Priority field is empty or missing",
      );
      expect(() => transformer["extractPriority"](null as any)).toThrow(
        "Priority field is empty or missing",
      );
      expect(() => transformer["extractPriority"](undefined as any)).toThrow(
        "Priority field is empty or missing",
      );
    });

    it("should throw error for invalid priority values", () => {
      expect(() => transformer["extractPriority"]("P4")).toThrow(
        "Invalid priority value: 'P4'. Expected P0, P1, P2, P3, or 0-3.",
      );
      expect(() => transformer["extractPriority"]("HIGH")).toThrow(
        "Invalid priority value: 'HIGH'. Expected P0, P1, P2, P3, or 0-3.",
      );
      expect(() => transformer["extractPriority"]("4")).toThrow(
        "Invalid priority value: '4'. Expected P0, P1, P2, P3, or 0-3.",
      );
    });
  });
  ...
  describe("normalizeTitle", () => {
    it("should trim whitespace from titles", () => {
      ...
      expect(transformer["normalizeTitle"]("  Multiple Spaces  ")).toBe(
        "Multiple Spaces",
      );
    });
    ...
    it("should preserve inner whitespace", () => {
      expect(
        transformer["normalizeTitle"]("Alert With  Multiple  Spaces"),
      ).toBe("Alert With  Multiple  Spaces");
    });
  ...
  describe("parseTimestamp", () => {
    ...
    it("should parse Date objects", () => {
      ...
      expect(transformer["parseTimestamp"](date)).toBe(
        "2025-09-16T10:30:00.000Z",
      );
    });

    it("should use current time for empty timestamps when not required", () => {
      expect(transformer["parseTimestamp"]("")).toBe(
        "2025-09-16T12:00:00.000Z",
      );
      expect(transformer["parseTimestamp"](null as any)).toBe(
        "2025-09-16T12:00:00.000Z",
      );
    });

    it("should throw error for empty timestamps when required", () => {
      expect(() => transformer["parseTimestamp"]("", true)).toThrow(
        "Required timestamp field is missing",
      );
      expect(() => transformer["parseTimestamp"](null as any, true)).toThrow(
        "Required timestamp field is missing",
      );
    });
    ...
    it("should validate timestamp length for security", () => {
      ...
      expect(() => transformer["parseTimestamp"](longTimestamp)).toThrow(
        "Timestamp string too long",
      );
    });
    ...
    it("should handle invalid timestamp formats gracefully when not required", () => {
      ...
      expect(transformer["parseTimestamp"]("invalid-date")).toBe(
        "2025-09-16T12:00:00.000Z",
      );
      expect(consoleSpy).toHaveBeenCalledWith(
        "Invalid timestamp format, using current time: invalid-date",
      );
    ...
    it("should throw error for invalid timestamp formats when required", () => {
      expect(() => transformer["parseTimestamp"]("invalid-date", true)).toThrow(
    ...
    it("should validate timestamp bounds for security", () => {
      ...
      expect(transformer["parseTimestamp"](tooOld)).toBe(
        "2025-09-16T12:00:00.000Z",
      );
      expect(transformer["parseTimestamp"](tooFuture)).toBe(
        "2025-09-16T12:00:00.000Z",
      );

      // Should throw when required
      expect(() => transformer["parseTimestamp"](tooOld, true)).toThrow(
        "Timestamp outside reasonable bounds",
      );
      expect(() => transformer["parseTimestamp"](tooFuture, true)).toThrow(
        "Timestamp outside reasonable bounds",
      );
    ...
    it("should throw error for invalid Date objects", () => {
      ...
      expect(() => transformer["parseTimestamp"](invalidDate)).toThrow(
        "Invalid Date object provided (contains NaN)",
      );
    });

    it("should throw error for invalid timestamp types", () => {
      expect(() => transformer["parseTimestamp"](123 as any)).toThrow(
        "Invalid timestamp type: 'number'",
      );
      expect(() => transformer["parseTimestamp"]({} as any)).toThrow(
        "Invalid timestamp type: 'object'",
      );
    });
  });
  ...
  describe("sanitizeString", () => {
    it("should remove potentially dangerous characters", () => {
      expect(
        transformer["sanitizeString"]('<script>alert("xss")</script>'),
      ).toBe("scriptalert(xss)/script");
      expect(
        transformer["sanitizeString"]("Test \"quotes\" and 'apostrophes'"),
      ).toBe("Test quotes and apostrophes");
      expect(transformer["sanitizeString"]("javascript:alert(1)")).toBe(
        "alert(1)",
      );
      expect(
        transformer["sanitizeString"](
  ...
  describe("validateUrl", () => {
    it("should validate and return valid HTTP/HTTPS URLs", () => {
      expect(transformer["validateUrl"]("https://example.com")).toBe(
        "https://example.com",
      );
      expect(
        transformer["validateUrl"]("http://example.com/path?query=1"),
      ).toBe("http://example.com/path?query=1");
    });

    it("should accept URLs without protocols and prepend https", () => {
      expect(transformer["validateUrl"]("www.example.com")).toBe(
        "https://www.example.com",
      );
      expect(transformer["validateUrl"]("example.com")).toBe(
        "https://example.com",
      );
      expect(transformer["validateUrl"]("subdomain.example.com/path")).toBe(
    ...
    it("should reject invalid protocols", () => {
      ...
      expect(
        transformer["validateUrl"]("data:text/html,<script>"),
      ).toBeUndefined();
    ...
    it("should handle invalid URL formats", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "Invalid hostname format: not-a-url",
      );
