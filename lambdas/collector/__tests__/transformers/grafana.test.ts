...
describe("GrafanaTransformer", () => {
  ...
  describe("transform", () => {
    ...
    it("should extract state correctly", () => {
      ...
      const resolvedPayload = {
        ...testGrafanaPayloads.resolved,
        alerts: [
          { ...testGrafanaPayloads.resolved.alerts[0], status: "resolved" },
        ],
      };

      expect(transformer.transform(firingPayload, mockEnvelope).state).toBe(
        "FIRING",
      );
      expect(transformer.transform(resolvedPayload, mockEnvelope).state).toBe(
        "RESOLVED",
      );
    });
  ...
  describe("error handling", () => {
    it("should throw error for invalid payload", () => {
      expect(() => transformer.transform(null, mockEnvelope)).toThrow(
        "Invalid Grafana payload: not an object",
      );

      expect(() => transformer.transform("invalid", mockEnvelope)).toThrow(
        "Invalid Grafana payload: not an object",
      );
    });
    ...
    it("should throw error for missing priority", () => {
      ...
      expect(() => transformer.transform(payload, mockEnvelope)).toThrow(
        /Missing required.*[Pp]riority/,
      );
    });
