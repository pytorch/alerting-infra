...
describe("CloudWatchTransformer", () => {
    ...
    it("should handle pipe-separated AlarmDescription format", () => {
      ...
      const customAlarm = {
        ...
        Message: JSON.stringify({
          ...alarmData,
          AlarmDescription:
            "TEAM=devops | PRIORITY=P1 | RUNBOOK=https://runbooks.example.com/cpu | Custom description",
        }),
      };
  ...
  describe("error handling", () => {
    it("should throw error for invalid JSON string payload", () => {
      expect(() => transformer.transform("invalid json", mockEnvelope)).toThrow(
        "Invalid CloudWatch payload: failed to parse JSON",
      );
    });
    ...
    it("should throw error for invalid SNS Message", () => {
      ...
      expect(() => transformer.transform(invalidSNS, mockEnvelope)).toThrow(
        "Invalid CloudWatch SNS Message: failed to parse",
      );
    });

    it("should throw error for non-object alarm data", () => {
      expect(() =>
        transformer.transform("not-json-string", mockEnvelope),
    ...
    it("should throw error for missing NewStateValue", () => {
      ...
      expect(() => transformer.transform(invalidAlarm, mockEnvelope)).toThrow(
        'Missing required field "NewStateValue"',
      );
    });
